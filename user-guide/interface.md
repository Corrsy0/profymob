...
#### Captcha Servisi
1. **Servis Seçimi**: Dropdown menüden captcha servisinizi seçin
   - 2captcha.com
   - anti-captcha.com
   - capsolver.com
   - capmonster.cloud

2. **API Key**: Servis API anahtarınızı girin

3. **Bakiye Kontrol**: Mevcut bakiyenizi görüntüleyin
   - ✅ Yeşil: Yeterli bakiye
   - ⚠️ Sarı: Düşük bakiye
   - ❌ Kırmızı: Yetersiz bakiye

#### Email Servisi
1. **Servis Seçimi**: Dropdown menüden email servisinizi seçin
   - xmailhub.net (Önerilir)
   - lution.ee
   - zeus-x.ru

2. **API Key**: Email servisi API anahtarınızı girin

3. **Bakiye Kontrol**: Email servisi bakiyenizi kontrol edin
...
2. **Proje Adı** girin (örn: "Web Sitesi 1", "Müşteri X")
...
- Toplam oluşturulan backlink sayısı
...
**Tema**:
- Açık tema
- Koyu tema (Gelecek güncellemede)
...
1. **Sık Kullanılan Ayarları Kaydedin**: "Ayarları Kaydet" özelliğini kullanın
2. **Proxy Listesi Hazırlayın**: İşlem öncesi `proxies.txt` dosyasını doldurun
3. **Toplu İşlemler**: Birden fazla platformda aynı anda çalışın
4. **Filtreleri Kullanın**: Raporlarda filtreleme ile hızlıca bulun

### Organizasyon

1. **Proje İsimlendirme**: Açıklayıcı isimler kullanın (örn: "Web1-Dev.to", "Müşteri2-Social")
2. **Düzenli Rapor Export**: Haftada bir Excel raporu alın
3. **Yedekleme**: `config.json` ve `reports.json` dosyalarını yedekleyin

### Performans

1. **Proxy Kullanın**: Toplu işlemlerde mutlaka proxy kullanın
2. **Yavaş İlerleyin**: Günlük 10-20 backlink yeterlidir
3. **Bakiye Takibi**: Servis bakiyelerinizi düzenli kontrol edin
4. **Platform Çeşitliliği**: Farklı platformlar kullanın
...
**Önceki**: [İlk Kullanım](../getting-started/first-use.md) | **Sonraki**: [Proje Yönetimi](projects.md)
